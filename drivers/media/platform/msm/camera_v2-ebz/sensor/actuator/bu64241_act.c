...
static int32_t bu64241_act_init(struct msm_actuator_ctrl_t *a_ctrl,
	...
	uint16_t	rev = 0;
	struct reg_settings_t init_settings_pp1[4]={
		{0xCC, 0x23},
		{0xD4, 0x01},
		{0xDF, 0xFF},
		{0xE4, 0x81},
	...
	CDBG("%s rev=0x%0x\n", __func__, rev);
	if((rev & 0x01) == 0){
		set_info->actuator_params.init_setting_size = 4;
	} 
#endif
...
#if defined(CONFIG_MACH_LYNX_GP9D)
			if((rev & 0x01) == 0){
				memcpy(init_settings, init_settings_pp1, set_info->actuator_params.init_setting_size * sizeof(struct reg_settings_t));
			} else {
#endif
