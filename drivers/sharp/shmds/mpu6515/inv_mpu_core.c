...
/* SHMDS_ID_1202_01 del S */
//msleep(100);
/* SHMDS_ID_1202_01 del E */
#else
	...
	if (st->chip_config.dmp_on && st->chip_config.enable) {
		...
		if (st->chip_config.display_orient_on)
			result |= inv_set_display_orient_interrupt_dmp(st,
								true);
		/* Restore gesture interrupt state */
...
/* SHMDS_ID_1101_01 add E */
	...
	if (st->chip_config.dmp_on && st->chip_config.enable) {
		...
		/* Only gesture interrupt is allowed during suspend to wake-up the host */
...
/* SHMDS_ID_1903_01 add E */
	/* add code according to different request End */
	mutex_unlock(&indio_dev->mlock);
/* SHMDS_ID_0112_01 del & add S */
//	mutex_lock(&st->suspend_resume_lock);
	st->suspend_state = false;
/* SHMDS_ID_0112_01 del & add E */
